...
Ниже — приёмы других «оптимизаторов» и причины, по которым Bamboo их
не повторяет. Осознанно и навсегда:

- «Очистка RAM». Так называемые чистилки памяти зовут `EmptyWorkingSet`,
  а он память не освобождает — он вытесняет рабочий набор приложения
  в файл подкачки на диске. Пользователь видит красивую цифру
  «освобождено 3 ГБ», после чего приложение считывает всё обратно
  за секунды. Памяти не прибавилось, зато добавилась лишняя запись
  на SSD и подтормаживание. Именно поэтому кнопки «очистить память»
  в Bamboo нет — сам Bamboo ничего в подкачку не выгружает.
