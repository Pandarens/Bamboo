...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Один агент на сеанс — и занимаем место первым делом, до окон, трея
    // и базы. Второй запуск не должен успеть ничего создать.
    //
    // Написано по случаю: на живой машине разом оказалось два агента —
    // два запуска ждали подтверждения прав и дождались оба. Две иконки
    // в трее, двойной расход и, что хуже, два писателя в одну базу.
    //
    ...
    let _single = match bamboo_sys::single::SingleInstance::acquire(AGENT_MUTEX) {
        ...
        Ok(None) => {
            ...
            // на ярлык, и правильный ответ — тихо уйти, оставив работать
            // того, кто уже запущен.
            println!("Bamboo уже работает — ищите панду в трее.");
            return Ok(());
        }
        Err(error) => {
            // Не смогли проверить — работаем. Отказать в запуске из-за
            // сбоя проверки было бы хуже самой беды, от которой она
            // защищает.
            eprintln!("проверить единственность не удалось: {error}");
            return run();
        }
    };
    run()
...
/// Имя места, которое занимает единственный агент.
///
/// `Local\` — на сеанс входа: у разных пользователей за одной машиной
