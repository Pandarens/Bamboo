...
pub fn install(release: &Release) -> (String, bool) {
    ...
    match replace_self(&bytes) {
        Ok(()) => (
            format!(
                "Версия {} установлена. Перезапускаю Bamboo…",
                release.version
            ),
            true,
        ),
...
/// Ключ запуска, с которым новая версия поднимается после обновления.
///
/// Нужен из-за защиты от второго агента: новая версия стартует, пока
/// старая ещё заканчивает работу и держит место единственного агента.
/// Без ключа новая увидела бы занятое место и тихо ушла — и после
/// обновления не осталось бы ни одной. С ключом она ждёт, пока старая
/// освободит место.
pub const AFTER_UPDATE_FLAG: &str = "--after-update";

/// Запускает только что установленную версию.
///
/// Прежде человеку предлагалось перезапустить Bamboo самому — закрыть
...
pub fn restart_into_new_version() -> Result<(), String> {
    ...
    std::process::Command::new(current)
        ...
        .map(|_| ())
        .map_err(|error| {
            format!("Запустить новую версию не удалось: {error}. Запустите Bamboo вручную.")
        })
}
