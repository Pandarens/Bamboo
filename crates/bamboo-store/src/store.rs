...
impl Store {
    ...
    /// Цела ли база прямо сейчас.
    ///
    /// Для проверки на ходу: повреждение, не задевшее запись, ошибкой
    /// не всплывает, и без проверки база неделю лежала битой при живом
    /// агенте.
    ...
    fn prepare(conn: Connection) -> Result<Store> {
        ...
        // FULL, и это вывод из трёх повреждений подряд. Здесь стояло NORMAL
        // с доводом «потерять последнюю транзакцию телеметрии не страшно».
        // Но терялась не транзакция — билась база: трижды за месяц, каждый
        // раз находка после утреннего включения, на здоровом диске. А журнал
        // действий рядом — тот же диск, тот же процесс, та же библиотека —
        // не повредился ни разу. Разница между ними одна: у журнала FULL.
        // Дешёвый накопитель без своей памяти, похоже, не держит порядок
        // записи при выключении, а NORMAL на этот порядок полагается.
        //
        // Довод про лишнюю запись на SSD отпал сам: запись идёт пачкой раз
        // в четыре часа, и fsync на такой коммит ничего не стоит.
