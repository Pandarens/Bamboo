...
impl Journal {
    ...
    fn prepare(conn: Connection) -> Result<Journal> {
        // Подождать занятую базу, а не падать сразу. К журналу обращаются
        // независимые участники — агент пишет действие, человек в тот же
        // момент открыл `bamboo journal`, — и без времени ожидания второй
        // получает «database is locked» вместо ответа через миллисекунды.
        // Вскрылось на раннере CI, где параллельные тесты столкнулись
        // на одном файле, но касается и живой машины.
