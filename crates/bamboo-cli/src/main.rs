...
//!
//! Все возможности наблюдателя из командной строки: снимки, трассы,
//! утечки, бюджет. Большинство команд только читают систему; исключения
//! названы прямо в справке — `optimize --apply` применяет действия,
//! `revert` откатывает их, `watchdog` делает проход сторожевого таймера.
//! Основной интерфейс для человека — агент с окном и треем.
